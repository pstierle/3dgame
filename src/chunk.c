...
Chunk *chunk_create(fnl_state noise, int world_offset_x, int world_offset_z)
{
    ...
    for (int x = 0; x < CHUNK_SIZE; ++x)
    {
        ...
        {
            float noise_val_y = fnlGetNoise3D(&noise, x + world_offset_x, 0, z + world_offset_z);
            ...
            for (int nx = -1; nx <= 1; ++nx)
            {
                for (int nz = -1; nz <= 1; ++nz)
                {
                    ...
                    float neighbor_noise_val_y = fnlGetNoise3D(&noise, x + world_offset_x + nx, 0, z + world_offset_z + nz);
                    float scaled_neighbor_noise_val_y = fmaxf(0.0f, fminf(128.0f, neighbor_noise_val_y * 60.0f + 64.0f));
                    int rounded_neighbor_noise_val_y = (int)roundf(scaled_neighbor_noise_val_y);

                    if (rounded_neighbor_noise_val_y < rounded_noise_val_y - 1)
                    {
                        for (int fill_y = rounded_neighbor_noise_val_y + 1; fill_y < rounded_noise_val_y; ++fill_y)
                        {
                            chunk->cube_positions[cube_positions_size][0] = x + world_offset_x;
...
{
    if (glm_vec3_distance(state.camera.position, chunk->position) < RENDER_DISTANCE)
    {
        chunk->visible = true;
    }
    else
...
void chunk_render(Chunk *chunk)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, chunk->cube_positions_size * sizeof(vec3), chunk->cube_positions, GL_DYNAMIC_DRAW);
